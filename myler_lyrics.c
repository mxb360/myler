...
static int LyricsWords_Compare(const void *l1, const void *l2)
{
    return ((MylerLyricsWords *)l1)->time - ((MylerLyricsWords *)l2)->time;
}

/* 歌词解析器，从文件file中解析歌词保存到MylerLyricsFile中 */
static int Lyrics_Lexer(MylerLyrics *lyrics, FILE *file)
{
    char buf[256];
    ...
    lyrics->lyrics = (MylerLyricsWords *)malloc(sizeof(struct _MylerLyricsWords) * MAX_LYRIC_LINE);
    ...
    lyrics->lyrics = realloc(lyrics->lyrics, sizeof(MylerLyricsWords) * lyrics->lyrics_cnt);
    lyrics->words = realloc(lyrics->words, sizeof(char *) * lyrics->words_cnt);
    /* 将生成的歌词按时间排序 */
    qsort(lyrics->lyrics, lyrics->lyrics_cnt, sizeof(MylerLyricsWords), LyricsWords_Compare);
...
MylerLyrics *MylerLyrics_GetLyricsByFile(const char *file_name)
{
    MylerLyrics *lyrics;
    FILE *file;

    lyrics = (MylerLyrics *)malloc(sizeof(struct _MylerLyrics));
    if (lyrics) {
        lyrics->file_name = file_name;
        lyrics->offset = 0;
        if ((file = fopen(file_name, "r")) == NULL || Lyrics_Lexer(lyrics, file)) {
            free(lyrics);
            lyrics = NULL;
        }
        if (file)
